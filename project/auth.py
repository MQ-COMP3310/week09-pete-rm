...
def login_post():
    ...
    # take the user
    #-supplied password and compare it with the stored password
...
def signup_post():
    ...
    user = db.session.execute(text('select * from user where email = "' + User.query.filter_by(email=email).first() +'"')).all()
